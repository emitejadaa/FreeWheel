-- Un dato único, una sola cuenta.
--
-- 1) Los emails ya cargados pasan a minúsculas y no vuelven a entrar de otra
--    forma (CHECK + índice único por expresión).
-- 2) Los teléfonos repetidos se liberan y "phone" pasa a ser único.
--
-- Los pasos que limpian datos son los mismos que
-- prisma/premigrate/003_account_identity_unique.sql, que corre antes de todo en
-- cada deploy: repetirlos acá no cuesta nada (son idempotentes) y hace que
-- `prisma migrate deploy` a secas —sin pasar por scripts/deploy-migrate.js—
-- también deje la base consistente antes de crear el índice.
DO $$
BEGIN
...
-- ── 5. El índice único de "phone" (el que Prisma espera del schema) ─────────
