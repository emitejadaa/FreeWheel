...
export class AllExceptionsFilter implements ExceptionFilter {
  private readonly logger = new Logger("ExceptionFilter");
    ...
    /*
      El código y la hora van al log Y a la respuesta, y son el mismo par. Es lo
      que permite que alguien diga "me dio UNEXPECTED a las 18:42" y que esa
      línea se encuentre sin adivinar.
    */
    const cuando = new Date().toISOString();
    this.logger.error(
      `${where} -> 500${who} [UNEXPECTED ${cuando}]: ${error?.message ?? "Unknown error"}`,
      error?.stack,
    );
    ...
    response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
      ...
      message:
        ...
        `registrado a las ${cuando}; pasá esa hora para encontrarlo.`,
      at: cuando,
    });
  }
