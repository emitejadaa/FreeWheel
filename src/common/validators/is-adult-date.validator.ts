 ...
 *
 * No tiene excepciones por cuenta. La tuvo mientras hubo una cuenta de prueba
 * que no llegaba a los 18, y eso significaba que el límite de edad de toda la
 * plataforma dependía de una variable de entorno: cualquiera que pudiera
 * escribirla podía bajar la mayoría de edad. El límite ahora es uno solo.
 */
...
export function IsAdultDate(validationOptions?: ValidationOptions) {
  return function (object: object, propertyName: string) {
    registerDecorator({
      ...
      validator: { validate: (value: unknown) => isAdultDate(value) },
    });
  };
}
