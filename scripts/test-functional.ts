...
const DEFAULT_TIMEOUT_MS = Number(
  process.env.FUNCTIONAL_TEST_TIMEOUT_MS ?? 10000,
);
...
async function main() {
  ...
  if (!targetUrl || !apiBaseUrl) {
    ...
    console.error(
      "  FRONTEND_URL=https://front.example.com API_BASE_URL=https://api.example.com npm run test:functional",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!auth?.accessToken) {
    results.push({
      ...
      status: "SKIP",
      detail:
        "No valid access token is available. Protected endpoint checks were not executed.",
    });
    printReport(results);
...
function printDetectedProjectShape(endpoints: DiscoveredEndpoint[]): void {
  const hasBackend =
    existsSync(join(process.cwd(), "src")) && endpoints.length > 0;
  ...
  const hasFrontend = frontendMarkers.some((marker) =>
    existsSync(join(process.cwd(), marker)),
  );

  console.log("Detected local project shape:");
  console.log(
    `- Backend: ${hasBackend ? "yes, NestJS controllers found" : "not detected"}`,
  );
  console.log(
    `- Frontend: ${hasFrontend ? "yes, frontend markers found" : "not detected in this repo"}`,
  );
  console.log("");
...
async function probeOpenApi(
  ...
  for (const candidate of candidates) {
    const response = await request(baseUrl, "GET", candidate, undefined, {
      optional: true,
    });
    if (
      response.status === 200 &&
      ...
      results.push({
        ...
        detail:
          "OpenAPI document detected. Local controller discovery is still used to avoid testing undocumented future features.",
      });
      return;
    }
  }
  ...
  results.push({
    ...
    status: "SKIP",
    detail:
      "No OpenAPI/Swagger document detected at /api-json, /openapi.json or /swagger.json.",
  });
}
...
async function probeCors(
  ...
  await record(
    ...
    async () => {
      ...
      if (!allowOrigin) {
        throw new Error(
          "Missing access-control-allow-origin header. Browser requests from the frontend may be blocked by CORS.",
        );
      }
    },
  );
...
async function authenticate(
  ...
  if (
    configuredEmail &&
    configuredPassword &&
    endpointSet.has("POST /auth/login")
  ) {
...
async function runProtectedFlow(
  ...
  await runIfDetected(
    ...
    async () => {
      const response = await request(baseUrl, "GET", "/users/me", undefined, {
        headers,
      });
      expectStatus(response, [200]);
      expectObjectWithId(
        response.body,
        "GET /users/me should return the authenticated user.",
      );
    },
  );
  ...
  await runIfDetected(
    ...
    endpointSet,
    "GET /vehicles/me",
    "List authenticated user vehicles",
    async () => {
      const response = await request(
        baseUrl,
        "GET",
  ...
  if (vehicleId && endpointSet.has("GET /vehicles/:id")) {
    await record(
      ...
      async () => {
        ...
        expectObjectWithId(
          response.body,
          "GET /vehicles/:id should return a vehicle.",
        );
      },
    );
  }
...
async function runVehicleDeleteCheck(
  ...
  if (
    !endpointSet.has("POST /vehicles") ||
    !endpointSet.has("DELETE /vehicles/:id")
  ) {
    results.push({
      name: "Delete vehicle",
      status: "SKIP",
  ...
  await record(
    results,
    "Create disposable vehicle for delete test",
    "POST",
    "/vehicles",
    async () => {
      const response = await request(
...
async function runListingFlow(
  ...
  if (!vehicleId || !endpointSet.has("POST /listings")) {
    results.push({
      ...
      status: "SKIP",
      detail:
        "No created vehicle is available or POST /listings is not implemented.",
    });
    return;
  }
  ...
  await runIfDetected(
    ...
    endpointSet,
    "GET /listings/me",
    "List authenticated user listings",
    async () => {
      const response = await request(
        baseUrl,
        "GET",
  ...
  if (listingId && endpointSet.has("GET /listings/:id")) {
    await record(
      ...
      async () => {
        ...
        expectObjectWithId(
          response.body,
          "GET /listings/:id should return a listing.",
        );
      },
    );
  }
...
async function runBookingFlow(
  ...
  await record(
    results,
    "Mark booking ready for pickup",
    "PATCH",
    "/bookings/:id/ready-for-pickup",
    async () => {
      const response = await request(
        baseUrl,
        "PATCH",
        `/bookings/${bookingId}/ready-for-pickup`,
        undefined,
        { headers: ownerHeaders },
      );
      expectStatus(response, [200]);
  ...
  await record(
    ...
    async () => {
      ...
      if (!pickupToken)
        throw new Error("pickupQrToken was not returned to renter.");
    },
  );

  await record(
    results,
    "Owner confirms pickup token",
    "POST",
    "/bookings/:id/confirm-pickup",
    async () => {
      const response = await request(
        baseUrl,
        "POST",
  ...
  await record(
    ...
    async () => {
      ...
      if (!returnToken)
        throw new Error("returnQrToken was not returned to owner.");
    },
  );

  await record(
    results,
    "Renter confirms return token",
    "POST",
...
async function registerFunctionalUser(
  ...
  await record(
    results,
    `Register ${roleLabel} functional user`,
    "POST",
    "/auth/register",
    async () => {
      const response = await request(baseUrl, "POST", "/auth/register", {
...
async function runOptionalHealth(
  baseUrl: string,
  results: TestResult[],
): Promise<void> {
  const response = await request(baseUrl, "GET", "/health", undefined, {
    optional: true,
  });
...
function classifyHttpError(response: HttpResult, expected?: number[]): string {
  const expectedText = expected
    ? ` Expected ${expected.join(" or")}, received ${response.status}.`
    : "";
