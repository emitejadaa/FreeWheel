...
export async function runEndpointChecks(
  ...
  for (const check of checks) {
    ...
    try {
      ...
      results.push({
        ...
        error: optionalMissingHealth
          ? "Optional health endpoint is not implemented."
          : undefined,
      });
    } catch (error) {
      results.push({
  ...
  if (
    frontendUrl &&
    normalizeBaseUrl(frontendUrl) !== normalizeBaseUrl(baseUrl)
  ) {
...
async function runCorsPreflightCheck(
  baseUrl: string,
  frontendUrl: string,
): Promise<CheckResult> {
  ...
  try {
    ...
    const ok =
      response.status === 204 && allowOrigin === normalizeBaseUrl(frontendUrl);
    const missingCorsOriginMessage =
      response.status === 404 && !allowOrigin
        ? " The deployment likely has no FRONTEND_URL/CORS_ORIGINS matching this frontend origin."
        : "";
...
export function printReport(baseUrl: string, results: CheckResult[]): void {
  ...
  for (const result of results) {
    const received =
      result.receivedStatus === null ? "ERROR" : String(result.receivedStatus);
    ...
    console.log(
      `${result.method.padEnd(7)} ${result.path.padEnd(14)} ${String(
        ...
        9,
      )} ${received.padEnd(9)} ${String(result.responseTimeMs).padEnd(9)} ${status}`,
    );
